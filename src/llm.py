...
def summarize_html_document(file_path: pathlib.Path):
    ...
    try:
        content = UnstructuredHTMLLoader(file_path, post_processors=[clean,remove_punctuation,clean_extra_whitespace]).load()
...
def reduce_document_summaries(summaries):
    try:
        prompt = reduce_documenet_summaries_prompt.format(docs="\n-----Document------\n".join(summaries))
